...
if st.session_state.result is None:
    st.markdown("## Convierte tu inventario en decisiones" if lang=="es" else "## Turn your inventory into decisions")
...
.block-container{max-width:1800px;padding:1.1rem 2.2rem 3rem 2.2rem;}
div[data-testid="stMetric"]{border:1px solid #E2E8F0;border-radius:8px;padding:12px 14px;background:#fff;}
</style>
""",unsafe_allow_html=True)
...
with h2:
    if st.button("Nuevo análisis" if lang=="es" else "New analysis",use_container_width=True):
        for _k in ["result","projection","validation_checks","category_detail"]:
