...
filter_label=f2.selectbox("Mostrar" if lang=="es" else "Show",
    ["Todos","Urgentes","Comprar","Exceso","Saludables"] if lang=="es" else ["All","Urgent","Buy","Excess","Healthy"])

filtered=view.copy()
if search:
    filtered=filtered[filtered["SKU"].astype(str).str.contains(search,case=False,na=False)]
filter_map={"Urgentes":1,"Urgent":1,"Comprar":2,"Buy":2,"Exceso":3,"Excess":3,"Saludables":4,"Healthy":4}
if filter_label in filter_map: filtered=filtered[filtered["_bucket"]==filter_map[filter_label]]

top=filtered.head(25).copy()
top["Prioridad"]=top["Priority_rank"]
top["SKU"]=top["SKU"].astype(str)
top["Situación"]=top["Action"]
top["Cantidad"]=top["Recommended_qty"].round(0).astype(int)
top["Impacto $"]=top["_impact"].round(0)
top["Quiebre"]=top["Stockout"].apply(fmt_date)
top["Confianza"]=top["Confidence"]
top["Por qué está priorizado"]=top["Priority_reason"]
display_cols=["Prioridad","SKU","Situación","Cantidad","Impacto $","Quiebre","Confianza","Por qué está priorizado"]
...
st.divider()
st.subheader("Abrir detalle de SKU" if lang=="es" else "Open SKU detail")
sku_options=filtered["SKU"].astype(str).tolist() if len(filtered) else view["SKU"].astype(str).tolist()
selected=st.selectbox("Selecciona un SKU" if lang=="es" else "Select SKU",sku_options)
open_detail=st.button("Ver detalle →" if lang=="es" else "View detail →",type="primary")
if open_detail:
...
if st.session_state.page=="detail":
    ...
    st.markdown(f"## {sku}  ·  #{int(r.Priority_rank)}")
