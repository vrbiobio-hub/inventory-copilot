...
def render_inventory_formula_example(view,lang="es"):
    """Use the #1 ranked SKU to explain exactly how the current engine calculated SS and ROP."""
    ...
    title="¿Cómo calculamos Safety Stock y Reorder Point?" if lang=="es" else "How do we calculate Safety Stock and Reorder Point?"
    with st.expander("ⓘ "+title,expanded=False):
        st.markdown((f"Usamos como ejemplo el **#1 del ranking: {sku}**. Los valores vienen del análisis de la base cargada."
                     if lang=="es" else
                     f"We use the **#1 ranked item: {sku}** as the example. Values come from the uploaded-data analysis."))
        ...
        c.metric("Lead Time",f"{lead:,.0f} "+("días" if lang=="es" else "days"))
        d.metric("Service Level",f"{service:.0%}")

        st.markdown("#### 1. Inventario de seguridad (Safety Stock)" if lang=="es" else "#### 1. Safety Stock")
        st.write(("Primero medimos cuánto ha variado históricamente la demanda. Después ajustamos esa variabilidad por el nivel de servicio y por el tiempo que debemos estar protegidos mientras esperamos reposición."
                  if lang=="es" else
                  "We first measure historical demand variability. We then adjust that variability for service level and the time that must be protected while replenishment arrives."))
        ...
        st.markdown((f"Para **{sku}**:  \n"
                     f"Z = **{z:.3f}** (Service Level {service:.0%})  \n"
                     f"Variabilidad histórica = **{std:,.1f} unidades/mes**  \n"
                     f"Período usado = **{months_in_lead:.2f} meses**  \n\n"
                     f"**Safety Stock = {z:.3f} × {std:,.1f} × √{months_in_lead:.2f} = {ss:,.0f} unidades**"
                     if lang=="es" else
                     f"For **{sku}**:  \n"
                     f"Z = **{z:.3f}** (Service Level {service:.0%})  \n"
                     f"Historical variability = **{std:,.1f} units/month**  \n"
                     f"Period used = **{months_in_lead:.2f} months**  \n\n"
                     f"**Safety Stock = {z:.3f} × {std:,.1f} × √{months_in_lead:.2f} = {ss:,.0f} units**"))

        st.markdown("#### 2. Punto para volver a comprar (ROP)" if lang=="es" else "#### 2. Reorder Point (ROP)")
        if pd.notna(rop) and lead>0:
            ...
            st.markdown((f"Demanda durante Lead Time = **{weekly:,.1f} × ({lead:,.0f}/7) = {demand_lt:,.0f} unidades**  \n"
                         f"Safety Stock = **{ss:,.0f} unidades**  \n\n"
                         f"**ROP = {demand_lt:,.0f} + {ss:,.0f} = {float(rop):,.0f} unidades**"
                         if lang=="es" else
                         f"Demand during Lead Time = **{weekly:,.1f} × ({lead:,.0f}/7) = {demand_lt:,.0f} units**  \n"
                         f"Safety Stock = **{ss:,.0f} units**  \n\n"
                         f"**ROP = {demand_lt:,.0f} + {ss:,.0f} = {float(rop):,.0f} units**"))
            if onhand <= ss:
                conclusion=(f"🔴 **{sku} tiene {onhand:,.0f} unidades y está por debajo de su Safety Stock ({ss:,.0f}).** Requiere revisión prioritaria."
                            if lang=="es" else
                            f"🔴 **{sku} has {onhand:,.0f} units and is below Safety Stock ({ss:,.0f}).** It requires priority review.")
            elif onhand <= float(rop):
                conclusion=(f"🟠 **{sku} tiene {onhand:,.0f} unidades y ya está bajo su ROP ({float(rop):,.0f}).** Es momento de revisar reposición."
                            if lang=="es" else
                            f"🟠 **{sku} has {onhand:,.0f} units and is already below its ROP ({float(rop):,.0f}).** Replenishment should be reviewed.")
            else:
                conclusion=(f"🟢 **{sku} tiene {onhand:,.0f} unidades y sigue sobre su ROP ({float(rop):,.0f}).** No ha cruzado todavía el punto de reposición."
                            if lang=="es" else
                            f"🟢 **{sku} has {onhand:,.0f} units and remains above its ROP ({float(rop):,.0f}).** It has not crossed the reorder point.")
            st.info(conclusion)
        else:
            st.warning("No calculamos ROP para este SKU porque falta Lead Time. Agrega ese dato para obtener un punto de reposición confiable." if lang=="es"
                       else "ROP is not calculated for this SKU because Lead Time is missing. Add it to obtain a reliable reorder point.")
        st.caption("La fórmula utiliza los mismos valores que usa el motor de Inventory Copilot; no es un ejemplo genérico separado del análisis." if lang=="es"
                   else "The formula uses the same values as the Inventory Copilot engine; this is not a generic example separate from the analysis.")
