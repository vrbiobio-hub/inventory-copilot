...
def analyze(sales,items,po,today=None,demand_change=0.,lead_delay=0,review_weeks=4):
    ...
    for _,it in items.iterrows():
        ...
        out.append(dict(SKU=sku,Situation=situation,Action=action,Confidence=conf,Forecast_weekly=weekly,Forecast_error=err,Bias=bias,On_hand=onhand,Unit_cost=cost,Currency=currency,Inventory_value=onhand*cost,Relevant_PO=relevant_po,Lead_time_days=lead,Safety_stock=safety,Reorder_point=reorder_point,Inventory_policy_status=policy_status,Recommended_qty=recommended,Purchase_value=recommended*cost,Excess_value=excess_value,First_risk=risk,Stockout=zero,Normal_arrival=arrival,Expedite=expedite,Explanation=explanation,Priority=priority,Model=model))
