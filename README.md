...
## V27 Safety Stock + Reorder Point
- Safety Stock and Reorder Point are first-class inventory-policy metrics.
- ROP = expected demand during lead time + safety stock.
- Missing lead time means ROP is shown as unavailable; the app asks for lead time instead of inventing a value.
- Plain-language statuses: Sobre ROP, Comprar / Bajo ROP, Bajo Safety Stock, Agregar Lead Time.
- Inventory and Actions now expose Safety Stock, ROP and status clearly.

## V28 Current Inventory in Summary + Actions
- Summary now surfaces total current on-hand inventory from the uploaded user dataset.
- Top 10 priorities show Current Inventory, Safety Stock and ROP together.
- Action Center and category details show Current Inventory beside Safety Stock and ROP.
- The UI explicitly states that Current Inventory is uploaded actual data, not a projection.

## V29 Formula Explainer
- Summary and Actions now include an expandable explanation of Safety Stock and Reorder Point.
- The example automatically uses the #1 ranked SKU from the user's current analysis.
- It shows the exact Service Level/Z factor, historical demand variability, lead time, forecast demand, Safety Stock and ROP used by the engine.
- It substitutes the SKU's actual calculated values into each formula and ends with a plain-language conclusion.
- The engine now exposes Service Level, Z factor and demand standard deviation to make the calculation auditable.

## V30 Why Buy + Missing Lead Time Guidance
- The #1 ranked SKU explanation now explicitly connects Safety Stock and ROP to the purchase recommendation shown in the table.
- Clarifies that ROP answers when to buy, while Suggested Buy answers how much to buy.
- Explains that suggested quantity is not simply ROP minus inventory; projected demand, available inventory, timely open POs, MOQ and order multiples can affect it.
- If Lead Time is missing, the explainer asks the user to add LEAD_TIME_DAYS and explains exactly what this unlocks.
- No ROP or replenishment-based quantity is presented as precise when Lead Time is unavailable.

## V31 Smart Minimum Data Matrix
- Adds a smart data-readiness matrix to Summary and Actions.
- Every major analysis says either Available or Cannot calculate — missing data.
- For unavailable calculations, the matrix states the minimum missing field and what adding it unlocks.
- Covers WOS, Safety Stock, ROP, Suggested Purchase Quantity, Financial Impact, Open PO Control and Supplier Analysis.
- Provides a short Recommended Next Step based on the user's actual missing fields.
- Optional fields such as MOQ and order multiple are separated from true minimum requirements.
- Missing data is never silently invented.

## V32 Decision Cards + No-Action Scenario
- Adds an executive conclusion in Summary.
- Adds a Decision Card for the #1 ranked SKU in Summary and Actions.
- Shows Data → Calculation → Recommendation traceability.
- Explains the main drivers behind each top recommendation.
- Adds a What happens if I do nothing? scenario using projected stockout and normal replenishment arrival.
- Shows confidence evidence such as demand history, inventory, lead time and open-PO availability.

## V33 Engine Reliability
- Demand frequency is now inferred per SKU (daily / weekly / monthly / custom) and normalized to weekly demand before WOS, Safety Stock, ROP, stockout and purchase calculations.
- Safety Stock, ROP and precise purchase quantity are no longer calculated when Lead Time is missing.
- Missing Lead Time still produces directional coverage-based guidance.
- PO quantities/dates and numeric item fields are defensively sanitized in the engine.
- Cost and Lead Time are no longer hard blockers in validation; missing fields reduce analysis capability instead.
- Added a permanent engine reliability suite covering frequency, missing data, dirty PO values, MOQ/multiples, demand changes, lead-time changes, PO timing, currencies and edge cases.
