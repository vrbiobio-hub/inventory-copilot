...
def _projection_guess_col(cols, aliases):
    norm={str(c).strip().lower():c for c in cols}
    for a in aliases:
        if a in norm: return norm[a]
    for c in cols:
        lc=str(c).strip().lower()
        if any(a in lc for a in aliases): return c
    return None

def build_projection_dataset(df):
    """Normalize simple future-demand files. Supports long rows or wide period columns."""
    x=df.dropna(axis=0,how="all").dropna(axis=1,how="all").copy()
    cols=list(x.columns)
    product=_projection_guess_col(cols,["producto","product","sku","item","material","descripcion","description","part number","part_number"])
    if product is None: product=cols[0]
    period=_projection_guess_col(cols,["periodo","period","fecha","date","mes","month","semana","week"])
    qty=_projection_guess_col(cols,["demanda proyectada","projected demand","forecast qty","forecast","cantidad proyectada","projected qty","demanda","demand","quantity","qty","cantidad"])
    onhand=_projection_guess_col(cols,["inventario actual","current inventory","on hand","on_hand","stock","inventory"])
    cost=_projection_guess_col(cols,["costo unitario","unit cost","cost","costo"])
    lead=_projection_guess_col(cols,["lead time","lead_time","tiempo entrega","dias entrega","delivery time"])

    if period is not None and qty is not None and period!=qty:
        out=pd.DataFrame({"Product":x[product].astype("string").str.strip(),"Period":x[period],"Projected_demand":pd.to_numeric(x[qty],errors="coerce")})
    else:
        exclude={product,onhand,cost,lead}
        candidates=[c for c in cols if c not in exclude]
        numeric=[]
        for c in candidates:
            vals=pd.to_numeric(x[c],errors="coerce")
            if vals.notna().mean()>=0.5: numeric.append(c)
        if not numeric: return pd.DataFrame(),{},""
        out=x[[product]+numeric].melt(id_vars=[product],var_name="Period",value_name="Projected_demand").rename(columns={product:"Product"})
        out["Projected_demand"]=pd.to_numeric(out["Projected_demand"],errors="coerce")
    out=out.dropna(subset=["Product","Projected_demand"])
    out=out[out["Product"].astype(str).str.strip()!=""]
    out["Projected_demand"]=out["Projected_demand"].clip(lower=0)
    # Preserve user period order rather than pretending unknown labels are dates.
    order=list(dict.fromkeys(out["Period"].astype(str).tolist()))
    out["Period"]=pd.Categorical(out["Period"].astype(str),categories=order,ordered=True)
    extras={}
    for key,col in [("On_hand",onhand),("Unit_cost",cost),("Lead_time_days",lead)]:
        if col is not None:
            tmp=x[[product,col]].copy(); tmp[col]=pd.to_numeric(tmp[col],errors="coerce")
            extras[key]=tmp.dropna(subset=[col]).drop_duplicates(product,keep="last").set_index(product)[col].to_dict()
    return out,extras,str(product)

def render_projection_workspace(df, extras, lang):
    base=df.groupby(["Product","Period"],observed=True)["Projected_demand"].sum().reset_index()
    products=base["Product"].nunique(); periods=base["Period"].nunique(); total=float(base["Projected_demand"].sum())
    by_period=base.groupby("Period",observed=True)["Projected_demand"].sum().reset_index()
    peak=by_period.loc[by_period["Projected_demand"].idxmax()]
    st.markdown("## Proyección" if lang=="es" else "## Projection")
    st.caption("Basado en la demanda futura que proporcionaste; no representa ventas reales." if lang=="es" else "Based on the future demand you provided; it does not represent actual sales.")
    a,b,c,d=st.columns(4)
    a.metric("Productos" if lang=="es" else "Products",f"{products:,}")
    b.metric("Demanda total" if lang=="es" else "Total demand",f"{total:,.0f}")
    c.metric("Períodos" if lang=="es" else "Periods",f"{periods:,}")
    d.metric("Mayor demanda" if lang=="es" else "Peak demand",f"{float(peak['Projected_demand']):,.0f}",str(peak['Period']))
    st.subheader("Demanda proyectada" if lang=="es" else "Projected demand")
    chart=by_period.rename(columns={"Period":"Período" if lang=="es" else "Period","Projected_demand":"Demanda" if lang=="es" else "Demand"}).set_index("Período" if lang=="es" else "Period")
    st.bar_chart(chart,use_container_width=True)

    scenario=st.segmented_control("Escenario" if lang=="es" else "Scenario",["−20%","−10%","Base","+10%","+20%"],default="Base",key="projection_scenario")
    mult={"−20%":.8,"−10%":.9,"Base":1.0,"+10%":1.1,"+20%":1.2}.get(scenario,1.0)
    scen=base.copy(); scen["Need"]=scen["Projected_demand"]*mult
    need_by_product=scen.groupby("Product",observed=True)["Need"].sum().sort_values(ascending=False)
    st.subheader("Lo que necesitarás" if lang=="es" else "What you'll need")
    x1,x2,x3=st.columns(3)
    x1.metric("SKUs necesarios" if lang=="es" else "SKUs needed",f"{products:,}")
    x2.metric("Unidades proyectadas" if lang=="es" else "Projected units",f"{need_by_product.sum():,.0f}")
    top_n=max(1,min(products,int((need_by_product.cumsum()/max(need_by_product.sum(),1)<=.8).sum()+1)))
    x3.metric("SKUs concentran ~80%" if lang=="es" else "SKUs make up ~80%",f"{top_n:,}")

    if extras.get("On_hand"):
        current=pd.Series(extras["On_hand"],dtype=float).reindex(need_by_product.index).fillna(0)
        gap=(need_by_product-current).clip(lower=0)
        covered=(need_by_product-gap).sum()/max(need_by_product.sum(),1)
        st.markdown(f"### {covered:.0%} " + ("de la proyección está cubierta por el inventario informado" if lang=="es" else "of projected demand is covered by the inventory provided"))
        st.caption((f"Gap estimado: {gap.sum():,.0f} unidades." if lang=="es" else f"Estimated gap: {gap.sum():,.0f} units."))
        if extras.get("Unit_cost"):
            costs=pd.Series(extras["Unit_cost"],dtype=float).reindex(gap.index).fillna(0)
            st.caption((f"Valor estimado del gap: ${(gap*costs).sum():,.0f}." if lang=="es" else f"Estimated gap value: ${(gap*costs).sum():,.0f}."))

    st.subheader("Productos clave" if lang=="es" else "Key products")
    top=need_by_product.head(5)
    for prod,val in top.items():
        st.markdown(f"**{prod}** · {val:,.0f} " + ("unidades" if lang=="es" else "units"))

    st.divider(); st.subheader("Conclusión de Inventory Copilot" if lang=="es" else "Inventory Copilot conclusion")
    peak_period=str(peak["Period"]); peak_qty=float(peak["Projected_demand"])*mult
    headline=(f"Tu mayor necesidad ocurrirá en {peak_period}." if lang=="es" else f"Your highest need occurs in {peak_period}.")
    st.markdown(f"### {headline}")
    st.write((f"El escenario seleccionado requiere aproximadamente {need_by_product.sum():,.0f} unidades en {products} productos; el período de mayor demanda alcanza {peak_qty:,.0f} unidades." if lang=="es" else f"The selected scenario requires about {need_by_product.sum():,.0f} units across {products} products; peak-period demand reaches {peak_qty:,.0f} units."))
    if extras.get("On_hand"):
        current=pd.Series(extras["On_hand"],dtype=float).reindex(need_by_product.index).fillna(0); gap=(need_by_product-current).clip(lower=0)
        priority=gap.sort_values(ascending=False).index[0] if gap.sum()>0 else None
        if priority:
            st.markdown((f"**Prioridad:** revisa primero **{priority}**, que presenta el mayor gap frente a la proyección." if lang=="es" else f"**Priority:** review **{priority}** first; it has the largest gap versus projected demand."))
        else:
            st.markdown("**Prioridad:** el inventario informado cubre la demanda total del escenario; revisa el timing por período." if lang=="es" else "**Priority:** reported inventory covers total scenario demand; review timing by period.")
    else:
        st.markdown((f"**Prioridad:** planifica primero los productos de mayor volumen antes de {peak_period}." if lang=="es" else f"**Priority:** plan the highest-volume products first before {peak_period}."))
    with st.expander("Ver todos los productos" if lang=="es" else "View all products"):
        table=need_by_product.rename("Unidades proyectadas" if lang=="es" else "Projected units").reset_index()
...
if st.session_state.result is None:
    _mode=st.segmented_control("",["Inventario","Proyección"] if lang=="es" else ["Inventory","Projection"],default="Inventario" if lang=="es" else "Inventory",key="entry_mode")
    _projection_mode=_mode in ["Proyección","Projection"]
    if _projection_mode:
        st.markdown("<div style='text-align:center;margin:.4rem 0 1.2rem'><h1 style='font-size:2.55rem;margin-bottom:.35rem'>Planifica lo que viene.</h1><p style='color:#64748b;font-size:1.08rem'>Sube tu demanda, proyecto o escenario futuro. Inventory Copilot calcula qué necesitarás.</p></div>" if lang=="es" else "<div style='text-align:center;margin:.4rem 0 1.2rem'><h1 style='font-size:2.55rem;margin-bottom:.35rem'>Plan what comes next.</h1><p style='color:#64748b;font-size:1.08rem'>Upload future demand, a project, or a scenario. Inventory Copilot calculates what you'll need.</p></div>",unsafe_allow_html=True)
        _plang=st.selectbox("Idioma / Language",["Español","English"],index=0 if st.session_state.get("ui_language","Español")=="Español" else 1,key="projection_language")
        st.session_state.ui_language=_plang; lang="es" if _plang=="Español" else "en"
        pf=st.file_uploader("Sube tu archivo de proyección" if lang=="es" else "Upload your projection file",type=["xlsx","xls","csv","tsv","txt"],key="projection_upload",help="Producto + demanda futura es suficiente. También puedes incluir inventario actual, costo o lead time." if lang=="es" else "Product + future demand is enough. You can also include current inventory, cost, or lead time.")
        if pf is None:
            st.caption("No necesitas una plantilla. Aceptamos períodos en filas o meses/semanas en columnas." if lang=="es" else "No template required. Periods can be in rows or months/weeks in columns.")
            st.stop()
        psheets=read_any(pf); pdf=list(psheets.values())[0].dropna(axis=0,how="all").dropna(axis=1,how="all")
        pdata,pextras,_=build_projection_dataset(pdf)
        if pdata.empty:
            st.error("No pude identificar demanda futura. Incluye producto y cantidades proyectadas por período." if lang=="es" else "I couldn't identify future demand. Include a product and projected quantities by period.")
            st.stop()
        st.success((f"✓ Detecté {pdata['Product'].nunique()} productos y {pdata['Period'].nunique()} períodos de proyección." if lang=="es" else f"✓ Detected {pdata['Product'].nunique()} products and {pdata['Period'].nunique()} projection periods."))
        if st.button("Analizar proyección" if lang=="es" else "Analyze projection",type="primary",use_container_width=True):
            st.session_state.projection_input=pdata; st.session_state.projection_extras=pextras; st.session_state.show_projection=True; st.rerun()
        if st.session_state.get("show_projection") and isinstance(st.session_state.get("projection_input"),pd.DataFrame):
            render_projection_workspace(st.session_state.projection_input,st.session_state.get("projection_extras",{}),lang)
        st.stop()
...
if "workspace" not in st.session_state or st.session_state.workspace not in ["Overview","Products","Copilot"]:
    st.session_state.workspace="Overview"
if "selected_product" not in st.session_state: st.session_state.selected_product=None

nav1,nav2,nav3=st.columns(3)
for col,name,label in [(nav1,"Overview","Overview" if lang=="en" else "Resumen"),(nav2,"Products","Products" if lang=="en" else "Productos"),(nav3,"Copilot","Copilot")]:
    with col:
        if st.button(label,type="primary" if st.session_state.workspace==name else "secondary",use_container_width=True,key=f"v31_{name}"):
...
if workspace=="Overview":
    ...
    st.divider()
    st.subheader("Inventory Copilot conclusion" if lang=="en" else "Conclusión de Inventory Copilot")
    if attention==0:
        _headline="No immediate action is needed." if lang=="en" else "No necesitas tomar una acción inmediata."
        _body="The products analyzed look healthy based on recent demand and current inventory." if lang=="en" else "Los productos analizados se ven saludables según la demanda reciente y el inventario actual."
        _priority="Keep monitoring changes in demand and inventory." if lang=="en" else "Continúa monitoreando cambios en demanda e inventario."
    elif overstock>=max(running,reorder) and overstock>0:
        _headline="Your main opportunity is reducing excess inventory." if lang=="en" else "Tu principal oportunidad está en reducir exceso de inventario."
        _body=(f"{overstock} products show high stock versus recent demand. {reorder} need replenishment soon and {running} are running low." if lang=="en" else f"{overstock} productos muestran inventario alto frente a su demanda reciente. {reorder} requieren reposición próxima y {running} tienen inventario bajo.")
        _priority="Review the highest-coverage products before placing new purchases." if lang=="en" else "Revisa primero los productos con mayor cobertura antes de generar nuevas compras."
    elif running>0:
        _headline="Your first priority is avoiding stockouts." if lang=="en" else "Tu primera prioridad es evitar quiebres de inventario."
        _body=(f"{running} products are running low and {reorder} more should be reviewed for replenishment." if lang=="en" else f"{running} productos tienen inventario bajo y {reorder} adicionales deberían revisarse para reposición.")
        _priority="Review the lowest-coverage products first." if lang=="en" else "Revisa primero los productos con menor cobertura."
    else:
        _headline="A small group of products needs replenishment attention." if lang=="en" else "Un grupo pequeño de productos necesita atención de reposición."
        _body=(f"{reorder} products should be reviewed soon; the rest do not show an immediate shortage." if lang=="en" else f"{reorder} productos deberían revisarse pronto; el resto no muestra una falta inmediata.")
        _priority="Start with the products closest to low inventory." if lang=="en" else "Comienza por los productos más cercanos a inventario bajo."
    st.markdown(f"### {_headline}")
    st.write(_body)
    st.markdown(("**Priority:** " if lang=="en" else "**Prioridad:** ")+_priority)

    if attention>0:
        with st.expander((f"View {attention} recommended actions" if lang=="en" else f"Ver {attention} acciones recomendadas"),expanded=False):
            _actions=view[view["Simple_status"]!="Healthy"].copy()
            _status_es={"Running low":"Inventario bajo","Reorder soon":"Comprar pronto","Too much stock":"Demasiado inventario"}
            for _i,(_,r) in enumerate(_actions.iterrows(),1):
                _status=r["Simple_status"]; _w=r.get("WOS")
                if _status=="Running low": _act="Replenish now" if lang=="en" else "Revisar reposición ahora"
                elif _status=="Reorder soon": _act="Plan replenishment" if lang=="en" else "Planificar reposición"
                else: _act="Review next purchase" if lang=="en" else "Revisar próxima compra"
                _coverage=(f"{_w:.1f} WOS" if pd.notna(_w) else "")
                c1,c2=st.columns([5,1])
                with c1:
                    st.markdown(f"**{_i}. {r['Description']}** · {_status if lang=='en' else _status_es.get(_status,_status)} · {_coverage}  \n{_act}")
                with c2:
                    if st.button("View →" if lang=="en" else "Ver →",key=f"conclusion_{r['SKU']}",use_container_width=True):
                        st.session_state.selected_product=str(r["SKU"]); st.rerun()
...
# ---------- Products ----------
    ...
    # Keep canonical status values in English internally, but localize everything the user sees.
    status_labels={
        "en":{"All":"All","Running low":"Running low","Reorder soon":"Reorder soon","Too much stock":"Too much stock","Healthy":"Healthy"},
        "es":{"All":"Todos","Running low":"Inventario bajo","Reorder soon":"Comprar pronto","Too much stock":"Demasiado inventario","Healthy":"Saludable"},
    }
    opts=["All","Running low","Reorder soon","Too much stock","Healthy"]
    status=q2.selectbox("Status" if lang=="en" else "Estado",opts,format_func=lambda x:status_labels[lang][x])
    pv=view.copy()
    ...
    for _,r in pv.head(100).iterrows():
        ...
        b.write(status_labels[lang].get(r["Simple_status"],r["Simple_status"]))
