...
if workspace in ["Resumen","Summary"]:
    ...
    missing_cost=int((pd.to_numeric(view.get("Unit_cost",0),errors="coerce").fillna(0)<=0).sum())
    missing_curr=int(view["Currency"].astype("string").fillna("").str.strip().eq("").sum()) if "Currency" in view else len(view)
    missing_lt=int((pd.to_numeric(view.get("Lead_time_days",0),errors="coerce").fillna(0)<=0).sum())
