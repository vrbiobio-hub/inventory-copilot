...
def analyze(sales,items,po,today=None,demand_change=0.,lead_delay=0,review_weeks=4):
    ...
    for _,it in items.iterrows():
        ...
        onhand=float(it.get('ON_HAND',0)); cost=float(it.get('UNIT_COST',0)); moq=float(it.get('MOQ',0) or 0); multiple=float(it.get('ORDER_MULTIPLE',1) or 1)
        ...
        out.append(dict(SKU=sku,Situation=situation,Action=action,Confidence=conf,Forecast_weekly=weekly,Forecast_error=err,Bias=bias,On_hand=onhand,Relevant_PO=relevant_po,Lead_time_days=lead,Safety_stock=safety,Recommended_qty=recommended,Purchase_value=recommended*cost,Excess_value=excess_value,First_risk=risk,Stockout=zero,Normal_arrival=arrival,Expedite=expedite,Explanation=explanation,Priority=priority,Model=model))
